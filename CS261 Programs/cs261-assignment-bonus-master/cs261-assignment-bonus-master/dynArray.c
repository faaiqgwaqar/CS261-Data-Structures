...
void addHeap(DynArr *heap, TYPE node)
{
  	/* FIXME */
		...
		if(pos > 0)
			parentNode = heap->data[pidx];
		while(pos > 0 && node.priority < parentNode.priority){
			heap->data[pos] = parentNode;
			pos = pidx;
			pidx = (pos - 1)/2;
			if(pos > 0)
				parentNode = heap->data[pidx];
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
  	/* FIXME */
		...
		if(rightIndex < max){
			smallIndex = _smallerIndexHeap(heap, leftIndex, rightIndex);
			if(heap->data[smallIndex].priority < heap->data[pos].priority){
				swapDynArr(heap, pos, smallIndex);
				_adjustHeap(heap, max, smallIndex);
			}
		}
		else if(leftIndex < max){
			if(heap->data[leftIndex].priority < heap->data[pos].priority){
