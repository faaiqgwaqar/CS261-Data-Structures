...
int FindMinPath(struct AVLTree *tree, TYPE *path)
{
        ...
        if (tree->cnt > 1){
               ...
               preorder(current, &min_cost, path, &path_len, candidate_path, &c_path_len, 0, tree->root->val);
               /* FIX ME */
...
Input argument:
     ...
     min_cost = the latest minimum cost0y
...
void printBreadthFirstTree(struct AVLTree *tree){
   ...
   /* FIX ME */
   if(EQ(tree->root,NULL)){
     free(queue);
     return;
   }
   queue[start] = tree->root; /*Enque the Root*/
   while(!EQ(queue[start],NULL)){
     current = queue[start];
     printf("%d ",current->val);
     if(current->left != NULL){
       queue[end+1] = current->left;
       end++;
     }
     if(current->right != NULL){
       queue[end+1] = current->right;
       end++;
     }
     queue[start] = NULL;
     start++;
